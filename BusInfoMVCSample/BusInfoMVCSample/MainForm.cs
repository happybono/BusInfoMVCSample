...
namespace BusInfoMVCSample.Views
{
    public partial class MainForm : Form
    {
        ...
        public MainForm(List<string> routeIds, List<string> bstopIds)
        {
            ...
            var setupForm = new FrmSetup();
            _setupController = new SetupController(setupForm);
            _setupController.SettingsSaved += SetupController_SettingsSaved;
        ...
        private async void SetupController_SettingsSaved(object sender, EventArgs e)
        {
            // Update the bus info, reset the timer, and update the labels when settings are saved
            await UpdateBusInfo();
            await UpdateBusStopLabels(); // Update labels with new settings
            ResetTimer();
        }
